...
class CorpusAligner(AcousticCorpusPronunciationMixin, AlignMixin, FileExporterMixin):
    ...
    def _align(self):
        # commented by claude: Determine whether to run speaker adaptation (fMLLR).
        ...
        if perform_speaker_adaptation:
            ...
            # The first-pass alignment uses the speaker-independent model (final.alimdl)
            # to produce initial alignments that are good enough to estimate fMLLR transforms.
        ...
        # commented by claude: Delete any stale fMLLR transform archives from previous runs.
        # "trans" ark files store per-speaker feature transforms; if left over from a
        # prior run they would corrupt the new alignment.
        for j in self.jobs:
        ...
        # commented by claude: FIRST PASS — speaker-independent GMM alignment.
        # Dispatches AlignFunction workers across all jobs in parallel.
        # Each worker reads feature arks and FST graphs, runs Kaldi's align-gmm-compiled,
        # and writes ali.ark (phone-state alignment) + words.ark + likelihoods.ark.
        ...
        if (
            ...
            # commented by claude: Estimate per-speaker fMLLR (feature-space Maximum
            # Likelihood Linear Regression) transforms from the first-pass alignments.
            # fMLLR finds a linear transform of the MFCC features that best matches
            # this speaker to the acoustic model's training distribution.
            self.calc_fmllr()
            if final_alignment:
            ...
            # commented by claude: SECOND PASS — speaker-adapted alignment.
            # The feature pipeline now applies the fMLLR transform on top of CMVN,
            # giving the aligner speaker-normalized features that should better match
            # the SAT model's Gaussians, improving alignment accuracy.
            self.align_utterances()
    ...
    def align(self) -> None:
        ...
        # commented by claude: Create (or resume) the alignment workflow record in the DB.
        # If this exact workflow was already completed (wf.done == True), skip all work.
        # This allows MFA to resume interrupted runs without re-doing completed stages.
        ...
        try:
            ...
            # commented by claude: STAGE 1 — Compile training graphs (FSTs).
            # For each utterance, combine the lexicon FST (L), grammar FST (G),
            # and acoustic model topology (H + C) into a single HCLG graph.
            # These per-utterance FSTs are saved as .ark files and are the inputs
            # to the Kaldi aligner. This is analogous to Kaldi's compile-train-graphs.
            self.compile_train_graphs()

            # commented by claude: STAGE 2 — Run GMM alignment (and optionally fMLLR).
            # Overridden in PretrainedAligner to also run fine_tune_alignments().
            self._align()

            # commented by claude: STAGE 3 — Extract phone/word intervals from Kaldi arks.
            # Reads the ali.ark files produced in stage 2, converts state sequences to
            # phone labels and time boundaries (CTM format), maps them to pronunciations
            # via the lexicon, and bulk-inserts phone_interval and word_interval rows
            # into the database. After this step, alignment data is fully in SQL.
            self.collect_alignments()

            # commented by claude: Mark the workflow as complete so future runs can skip it.
            with self.session() as session:
    ...
    def collect_alignments(self) -> None:
        ...
        with self.session() as session:
            ...
            # commented by claude: Idempotency guard — if alignments were already extracted
            # (e.g. a previous run completed this step), skip the entire method to avoid
            # duplicate rows in the phone_interval and word_interval tables.
            if workflow.alignments_collected:
                return

            # commented by claude: These index definitions are applied AFTER the bulk insert.
            # Dropping indexes before the insert and recreating them afterward is a standard
            # bulk-load optimization: inserting millions of rows without indexes is much
            # faster because the DB does not have to update the index B-trees on every row.
            ...
            if config.USE_POSTGRES:
                # commented by claude: PostgreSQL triggers (e.g. foreign key checks,
                # auto-update triggers) are disabled here for performance during the
                # massive bulk insert of potentially millions of interval rows.
                # Indexes are also dropped and will be rebuilt below.
                session.execute(sqlalchemy.text("ALTER TABLE word_interval DISABLE TRIGGER all"))
                session.execute(sqlalchemy.text("ALTER TABLE phone_interval DISABLE TRIGGER all"))
        ...
        new_words = []
        # commented by claude: Two different bulk-insert strategies are used depending on
        # the backend. PostgreSQL uses COPY FROM (streaming CSV directly into the table via
        # a raw connection cursor) — the fastest possible insert method. SQLite instead
        # writes a CSV file and calls the sqlite3 CLI tool's .import command, which is
        # faster than SQLAlchemy ORM inserts for large row counts.
        if config.USE_POSTGRES:
            conn = self.db_engine.raw_connection()
            cursor = conn.cursor()
            word_buf = io.StringIO()   # in-memory CSV buffer streamed to Postgres COPY
            phone_buf = io.StringIO()  # in-memory CSV buffer streamed to Postgres COPY
        else:
        ...
        # commented by claude: Run AlignmentExtractionFunction in parallel across all jobs.
        # Each worker reads the ali.ark (phone-state sequences) and words.ark files
        # for its job, converts the Kaldi state IDs to phone labels and time boundaries
        # using the transition model and phone table, then maps phones back to word
        # pronunciations via the lexicon compiler. Yields (utterance_id, dict_id, ctm)
        # tuples where ctm contains a structured list of WordInterval objects, each
        # holding its phone-level CtmInterval children.
        for (
            utterance,
            dict_id,
            ctm,
            ...
            try:
                ...
                # commented by claude: Iterate over each aligned word's interval object.
                # word_interval.phones contains the child phone-level intervals.
                # We assign monotonically increasing IDs (max_word_interval_id, max_phone_interval_id)
                # ourselves instead of using DB auto-increment, because the bulk CSV
                # insert bypasses SQLAlchemy's ORM and the DB's sequence generator.
