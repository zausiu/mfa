...
class CompileTrainGraphsFunction(KaldiFunction):
    ...
    def _run(self):
        ...
        # commented by claude: Each worker opens its own DB session (thread-safe via scoped_session)
        # and a dedicated log file. thread_logger redirects kalpy's internal Kaldi logging
        # to a per-job .log file in working_log_directory so errors are easy to find.
        with self.session() as session, thread_logger(
            ...
            # commented by claude: When G2P is active, the text has been character-segmented
            # (e.g. "hello" → "h e l l o") so we use normalized_character_text as the
            # grammar input. Otherwise we use the standard word-level normalized_text.
            if self.use_g2p:
                text_column = Utterance.normalized_character_text
            else:
                text_column = Utterance.normalized_text

            # commented by claude: Iterate over each pronunciation dictionary assigned to
            # this job. In a monolingual corpus there is usually one dictionary, but MFA
            # supports multilingual corpora where different speakers use different dictionaries.
            for d in job.training_dictionaries:
                ...
                # commented by claude: TrainingGraphCompiler wraps Kaldi's compile-train-graphs.
                # It takes the acoustic model (to get the HMM transition topology + tree)
                # and the lexicon FST (L.fst) and compiles per-utterance HCLG graphs in
                # batches of 500 utterances for efficiency.
                ...
                # commented by claude: Query utterances assigned to this job and dictionary.
                # kaldi_id is formatted as "speaker_id-utterance_id" which Kaldi uses as
                # the archive key. Ignored utterances (too short, OOV-only, etc.) are excluded.
                query = (
                ...
                # commented by claude: fsts.ark is a Kaldi ARK archive file where each
                # entry is (kaldi_id → HCLG FST). This file is later read by AlignFunction.
                fst_ark_path = job.construct_path(
                    workflow.working_directory, "fsts", "ark", d.name
                )
                # commented by claude: The core graph compilation call. For each utterance,
...
class AlignFunction(KaldiFunction):
    ...
    def _run(self) -> None:
        ...
        with self.session() as session, thread_logger(
            ...
            # commented by claude: boost_silence inflates the acoustic probability of silence
            # phones by this factor before alignment. Values > 1.0 make the aligner prefer
            # silence at the edges of utterances (reduces over-alignment into silence regions).
            ...
            for d in job.training_dictionaries:
                ...
                # commented by claude: fsts.ark contains the pre-compiled HCLG graphs from
                # CompileTrainGraphsFunction. Each entry maps kaldi_id → FST.
                fst_path = job.construct_path(self.working_directory, "fsts", "ark", d.name)
                align_logger.debug(f"Training graph archive: {fst_path}")

                # commented by claude: Feature archive is the MFCC (+ CMVN + optional delta/LDA/
                # fMLLR) feature matrix for each utterance. construct_feature_archive builds
                # the Kaldi rspecifier string that chains the feature transforms in the right order.
                feature_archive = job.construct_feature_archive(self.working_directory, d.name)
                ...
                # log-likelihoods. All three are standard Kaldi ARK binary archives.
                ali_path = job.construct_path(self.working_directory, "ali", "ark", d.name)
                ...
                # commented by claude: Remove stale output files from a previous run to
                # prevent accidentally mixing old and new alignments in the same archive.
                ...
                # commented by claude: When using a SAT model, the first pass uses the
                # speaker-independent model (final.alimdl). Its outputs are written to
                # separate *_first_pass.ark files so that second-pass (SAT) outputs can
                # go to the canonical ali.ark/words.ark/likelihoods.ark paths without
                # overwriting the first-pass arks (which are needed for fMLLR estimation).
...
class AnalyzeAlignmentsFunction(KaldiFunction):
    ...
    def _run(self):
        ...
        with self.session() as session, thread_logger(
            ...
            # commented by claude: Load per-phone duration statistics (mean and std dev)
            # that were computed from the training data and stored in the Phone table.
            # These are used below to compute a Z-score for each phone's observed duration,
            # which identifies utterances with suspiciously short or long phone durations.
            phones = {
                k: (m, sd)
            ...
            for d in job.training_dictionaries:
                # commented by claude: Only analyze utterances that successfully aligned
                # (alignment_log_likelihood is not NULL). Failed utterances have no intervals.
                query = (
                ...
                for utterance in query:
                    ...
                    # commented by claude: Load the raw audio waveform for this utterance
                    # and compute RMS (root mean square) energy in short frames.
                    # Frame length = 25ms, hop = 10ms — matching the acoustic feature pipeline.
                    # RMS energy is used to compute SNR: speech energy vs. silence energy.
                    ...
                    for pi in phone_intervals:
                        # commented by claude: Convert absolute phone boundary times to
                        # RMS frame indices. The 0.01s hop size matches the feature frame shift.
                        begin_index = int((pi.begin - utterance.begin) / 0.01)
                        end_index = int((pi.end - utterance.begin) / 0.01)
                        if pi.phone_id == silence_phone_id:
                            # commented by claude: Accumulate energy in silence regions.
                            # High silence energy (relative to speech) means low SNR — a
                            # sign of noisy audio or misaligned silence boundaries.
                        ...
                        # commented by claude: Compute the absolute Z-score of this phone's
                        # duration relative to the population mean/stdev. A very high Z-score
                        # (e.g. > 10) indicates the phone was forced into an implausible duration,
                        # which is a strong signal of a misalignment or transcription error.
                    ...
                    # commented by claude: Convert linear RMS energy to dB scale.
                    # SNR = speech_dB - silence_dB. A high SNR means clean speech;
                    # low or negative SNR means the silence is as loud as the speech (noisy recording).
                    ...
                    # commented by claude: Yield quality metrics for this utterance back to
        ...
        with self.session() as session, thread_logger(
            ...
            for d in job.dictionaries:
                ...
                if self.transcription:
                    ...
                    # commented by claude: For forced alignment (not transcription), read the
                    # three Kaldi ARK files produced by AlignFunction:
                    #   ali.ark   — Viterbi state sequences (HMM state IDs per frame)
                    #   words.ark — word-level integer sequences
                    #   likelihoods.ark — per-frame acoustic log-likelihoods
                    ali_path = job.construct_path(workflow.working_directory, "ali", "ark", d.name)
                    if not ali_path.exists():
                        continue
                    words_path = job.construct_path(
                    ...
                    for alignment in alignment_archive:
                        ...
                        # same-phone frames into a single interval with begin/end times.
                        intervals = alignment.generate_ctm(
                            self.transition_model, lexicon_compiler.phone_table, self.frame_shift
                        )
                        utterance = int(alignment.utterance_id.split("-")[-1])
                        ...
                        try:
                            ...
                            # commented by claude: phones_to_pronunciations groups the flat
                            # phone interval list into word-level intervals by matching the
                            # phone sequence to the pronunciation entries in the lexicon.
                            # Returns a CtmResult with word_intervals, each containing a
                            # .phones list of child CtmInterval objects.
                            ctm = lexicon_compiler.phones_to_pronunciations(
                                alignment.words,
                                intervals,
                                transcription=False,
                            ...
                            # commented by claude: Shift interval timestamps from utterance-relative
                            # time (0-based) to absolute file time (utterance.begin-based)
                            # so that the stored begin/end values match what appears in the TextGrid.
                            ctm.update_utterance_boundaries(*utterance_times[utterance])
                            if text is not None:
                                # commented by claude: fix_unk_words corrects OOV word intervals
                                # by replacing the internal <unk> phone sequence with a single
                                # interval labeled with the original unknown word string.
                            ...
                            # commented by claude: Yield (utterance_id, dict_id, ctm) to the
                            # caller (collect_alignments) which will write this CTM data into
                            # the phone_interval and word_interval DB tables.
