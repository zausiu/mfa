...
class AlignMixin(DictionaryMixin):
    ...
    def compile_train_graphs(self) -> None:
        ...
        # commented by claude: Launch CompileTrainGraphsFunction workers in parallel.
        # Each worker builds one FST (Finite State Transducer) per utterance by composing
        # H (HMM topology) ∘ C (context dependency) ∘ L (lexicon) ∘ G (grammar/text).
        # The resulting per-utterance HCLG graphs are saved to fsts.ark files and are
        # the direct input to the Kaldi aligner. No return value is needed here;
        # the graphs are written to disk and read by AlignFunction in the next step.
    ...
    def align_utterances(self, training=False) -> None:
        ...
        # commented by claude: Dispatch AlignFunction workers in parallel (one per job).
        # Each worker runs Kaldi's GMM aligner on its subset of utterances.
        # The callback yields (utterance_kaldi_id, log_likelihood) per utterance.
        ...
        for utterance, log_likelihood in run_kaldi_function(
            ...
            try:
                if log_likelihood:
                    ...
                    num_errors += 1
                if not training:
                    # commented by claude: Collect (utterance_id, log_likelihood) pairs
                    # to be written back to the utterance table after all workers finish.
                    # The utterance kaldi_id is formatted as "speaker-utterance_id", so
                    # we parse the integer ID from the last segment after "-".
        ...
        if not training:
            ...
            with self.session() as session:
                ...
                # commented by claude: Normalize log-likelihoods by the number of frames
                # in each utterance. This gives a per-frame log-likelihood which is
                # comparable across utterances of different lengths, and is used in
                # analyze_alignments() to detect problematic utterances.
                ...
                # pass as the workflow's overall score. Useful for comparing quality across
                # training iterations when used during acoustic model training.
