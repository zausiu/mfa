...
class PretrainedAligner(TranscriberMixin, TopLevelMfaWorker):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # commented by claude: Open and parse the .zip acoustic model file. The model
        # contains final.mdl (GMM parameters), tree, phones.txt, and optional fMLLR
        # (speaker adaptation) model files. No Kaldi processes are launched here.
        self.acoustic_model = AcousticModel(acoustic_model_path)
        self.use_reference_alignments = use_reference_alignments

        # commented by claude: Pull the model's stored parameters (phone set, feature
        # config, speaker adaptation flag, etc.) and merge them with the user-supplied
        # kwargs so the parent class receives the full unified config dictionary.
        kw = self.acoustic_model.parameters
        kw.update(kwargs)
        super().__init__(**kw)
        self.fine_tune = fine_tune
        self.fine_tune = fine_tune_boundary_tolerance
        # commented by claude: final_alignment=True means this is the last alignment pass,
        # so Kaldi will write full alignment archives (ali, words, likelihoods) rather
        # than the lighter intermediate archives used during iterative training.
        self.final_alignment = True
        # commented by claude: KalpyAligner is a Python wrapper around Kaldi's aligner.
        # It is created in setup() once the acoustic model and lexicon are ready.
    ...
    def setup_acoustic_model(self) -> None:
        ...
        # that Kaldi's C++ binaries will read during graph compilation and alignment.
        self.acoustic_model.export_model(self.working_directory)
        os.makedirs(self.phones_dir, exist_ok=True)
        # commented by claude: Move phone/grapheme symbol tables to the dedicated phones_dir.
        # phones.txt maps phone labels → integer IDs (required by Kaldi FST operations).
        # graphemes.txt maps individual characters → IDs (used when use_g2p=True).
    ...
    def setup(self) -> None:
        ...
        # commented by claude: Guard against running setup() more than once in a session.
        # self.initialized is set to True at the end of this method.
        if self.initialized:
            return
        begin = time.time()
        try:
            # commented by claude: Create (or open existing) the SQLite/PostgreSQL database
            # and apply SQLAlchemy schema migrations. All corpus, dictionary, speaker,
            # utterance, and alignment data will be stored and queried through this DB.
            self.initialize_database()

            # commented by claude: Register a new "alignment" workflow entry in the
            # corpus_workflow table. This acts as the progress tracker — if the run
            # is interrupted and re-run, MFA checks wf.done to skip completed steps.
            ...
            # commented by claude: Extract acoustic model files to working_directory
            # and populate the phone/grapheme tables in the database. Must happen
            # before load_corpus() so the phone inventory is known when importing words.
            self.setup_acoustic_model()

            # commented by claude: Scan corpus_directory, parse all audio + transcript files,
            # and insert File, SoundFile, TextFile, Speaker, and Utterance rows into the DB.
            # Also parses the pronunciation dictionary and inserts Word/Pronunciation/Phone rows.
            # This is the most time-consuming setup step for large corpora.
            self.load_corpus()
    ...
    def _align(self):
        # commented by claude: If not using pre-existing reference (manual) alignments,
        # clear the manual_alignments flag on all utterances so the automatic aligner
        # can process them freely without being constrained by previous reference data.
        if not self.use_reference_alignments:
            self.reset_manual_alignments()

        # commented by claude: Delegate to CorpusAligner._align() which handles the
        # first-pass GMM alignment and — if the acoustic model supports it —
        # fMLLR speaker adaptation followed by a second-pass alignment.
        super()._align()
    ...
    def align(self) -> None:
        """Run the aligner"""
        # commented by claude: Two-step entry point: first run setup() to build the DB
        # and load the corpus, then call the parent CorpusAligner.align() which
        # orchestrates compile_train_graphs → _align → collect_alignments.
        self.setup()
        super().align()
    ...
    def fine_tune_alignments(self) -> None:
        ...
        with self.session() as session:
            ...
            update_mappings = []
            # commented by claude: Run FineTuneFunction in parallel across all jobs.
            # Each worker re-scores the audio around each phone boundary using the
            # acoustic model and finds the locally optimal boundary position within
            # fine_tune_boundary_tolerance seconds of the original boundary.
            for result in run_kaldi_function(
                FineTuneFunction, arguments, total_count=self.num_utterances
            ):
                update_mappings.extend(result)

            # commented by claude: Write refined phone boundary begin/end times back to
            # the phone_interval table in a single bulk update for efficiency.
            bulk_update(session, PhoneInterval, update_mappings)
            session.flush()

            # commented by claude: Word interval boundaries must stay consistent with their
            # child phone intervals. Recompute each word interval's begin/end as the
            # min(phone begins) and max(phone ends) across all its constituent phones.
        ...
        # commented by claude: After fine-tuning, phone boundaries are 10× more precise,
        # so the export frame shift (used for TextGrid rounding) is divided by 10
        # to match the new sub-frame precision of the stored intervals.
